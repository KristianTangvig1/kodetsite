...
]).then(([sunglasses, jewellery, watches]) => {
  const mixed = [
    sunglasses.products[0],
    jewellery.products[0],
    watches.products[0]
  ];

  heroContainer.innerHTML = mixed
    .map(p => `<img src="${p.thumbnail}" alt="${p.title}">`)
    .join("");
});
...
Promise.all([
  ...
  const mixed = [
    sunglasses.products[1],
    jewellery.products[1],
    mensBags.products[0],
    womensBags.products[0]
  ];
